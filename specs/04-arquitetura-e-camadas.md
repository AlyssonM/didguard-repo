...
2. **Firmware (ESP32 + RC522)**
   - Leitura/escrita NFC, MAC, contador e challenge-response.
3. **Relayer (Node.js/Express)**
...
### 4.1 Provisionamento
`Frontend -> Relayer(start) -> ESP32(status) -> TAG(write) -> Relayer(uploadDid) -> IPFS + Blockchain -> Relayer(finish) -> Frontend(events)`

### 4.2 Acesso por TAG
`ESP32(read) -> Relayer(challenge) -> ESP32(proof) -> Relayer(authorize) -> Blockchain(hasAccessDebug) -> ESP32(commit local + remote)`

### 4.3 Acesso por biometria
...
- CID no contrato, documento completo no IPFS.
- Relayer como writer único on-chain.
- Estado auxiliar local para busca rapida e resiliência operacional.
